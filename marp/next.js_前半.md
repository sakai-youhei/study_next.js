...
## 2-6. Page Router とは？ — 2022年まで主流だったルーティング
...
## 2-8. App Router：レンダリング戦略の"超"シンプル化
...
## 3-1 補足. Before / After で見る RSC の威力
...
## 3-8. 解決策：コンポジションパターン（childrenで受け取る）

> 3-7 との違い — **CCは SC を直接 `import` せず、`children` propsで「親から渡された要素を表示するだけ」の役割に変える**
...
## 3-9 補足. SC の実コード — `StaticServerDataFetch.tsx`

> 3-9 で `children` として渡していた SC の中身。**`async` + `fetch` だけ**で完結する
...
>
> 🔑 もし `ClientBoundary` 側が `<StaticServerDataFetch />` を直接 `import` していたら、ブラウザでこの fetch をやり直そうとして失敗する（= 3-7 のエラー）。**children 経由で受け取る** ことで「サーバーで取り終わったHTML」になり、安全に表示できる
...
- どんな書き方をすると Dynamic に切り替わるかを判別できる
- **RSC Payload** が何のために送られているのかを理解する
- `<Link>` による画面遷移がなぜ高速なのかを説明できる

> 💡 用語の前提：**CDN** = Content Delivery Network（世界中のサーバーから高速配信する仕組み）／ **Payload** = 通信で送られるデータ本体
...
- **CDN**（世界中のサーバーから高速配信する仕組み）にキャッシュ可能 → **究極の表示速度**
