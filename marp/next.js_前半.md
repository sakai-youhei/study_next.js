...
2. **環境構築と App Router の基礎** — モダンな開発環境とルーティングの考え方
...
- **Turbopack / Webpack** による高速バンドル（複数JSを1つにまとめるツール）
- **Tree-shaking**（使われていないコードを自動で削除）
- **コード分割**（ページごとに必要なJSだけを送る）
- 画像・フォントの最適化
...
> 💡 用語の前提：**バンドラー** = 多数のJSファイルを1つにまとめて軽量化するツール／ **ルーティング** = URL ごとに表示するページを決める仕組み
...
- どれだけ大規模なアプリでも **ミリ秒単位** でブラウザが更新される（**HMR** = Hot Module Replacement）
...
- **RSC**（React Server Component）が何をひっくり返したのかを理解する
- Server Component と Client Component で「使える機能」の違いを覚える
- `"use client"` の意味と、付けるべき場所を判断できる
- 陥りやすい「Client Boundary」エラーとその回避策を学ぶ
- **コンポジションパターン**で SC と CC を共存させる方法を知る

> 💡 用語の前提：**コンポーネント** = 画面を構成する再利用可能な部品／ **RSC** = サーバー側で実行・レンダリングされるコンポーネント
