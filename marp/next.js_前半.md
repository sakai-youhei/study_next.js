...
- ✅ App Router で Next.js プロジェクトを **0から構築・読解**できる
- ✅ **Server Component / Client Component** の役割を理解し、適切に使い分けられる
- ✅ **Static / Dynamic / ISR** を判別し、4層キャッシュを意識したデータ取得を書ける
- ✅ `layout.tsx` / `loading.tsx` / `error.tsx` / `route.ts` 等の**予約ファイル**で堅牢な構成を組める
- ✅ **Dynamic Segments / Parallel Routes / Intercepting Routes** で高度なUI設計ができる
- ✅ 本プロジェクトの実コードを読み解き、自分のプロジェクトに応用できる
...
6. **圧倒的な表示速度を支える4つのキャッシュ層** — 4層が連携する仕組み
...
- **Tree-shaking** — 使っていないコード（import したが呼ばれない関数等）を本番ビルドから自動除外
- **コード分割** — ページごとに必要なJSだけを切り出して配信（全ページ分を一括送信しない）
- **画像・フォントの最適化** — `next/image` `next/font` でサイズ縮小・WebP変換・遅延読み込みを自動化
- **Turbopack** による高速ビルド（旧来の Webpack 設定の手書きが不要に）
...
`npx create-next-app` を1行実行するだけで、Vercel が定めるベストプラクティスのツール群が最適な設定で自動セットアップされます。

> 💡 **Vercel** = Next.js を開発している米国の会社。Next.js本体のメンテナーであり、Next.jsアプリの**最適化されたホスティングサービス**（`vercel deploy` でワンコマンド本番公開）も提供。React の中心メンバーが多数在籍
...
## 2-2. 「Turbopack」による高速ビルド — 保存から反映までを瞬時に
...
## 2-6. パスエイリアス（`@/`）— 絶対パスで `import` する仕組み

> `@/` は本プロジェクトの **`src/`** を指す省略記法。深い階層でも `../../../` を書かずに済む

```tsx
// ❌ 相対パス：どこから書くかで `..` の数が変わる・移動でリンク切れ
import Button from "../../../components/Button"

// ✅ パスエイリアス：常に src/ 起点で書ける
import Button from "@/components/Button"
```

### 🔧 設定箇所（`tsconfig.json` の `paths`）

```json
"paths": { "@/*": ["./src/*"] }   // 👈 @/ を ./src/ にマッピング
```

**💡 メリット：**

- **可読性** — `@/components/Button` と書けば**どこから書いても同じ**
- **リファクタ耐性** — ファイル移動しても import パスが壊れにくい
- **エディタ補完が効く** — `@/` 入力で `src/` 配下のファイル候補が出る

> 🔑 `create-next-app` のデフォルトで `@/* → src/*` 設定済み。本プロジェクトも同じ規約
...
**【Before】CC時代 — `useEffect` + `useState` の煩雑さ：**
...
| **Server Action** | `'use server'` を付けた関数 | ✅ 定義 | ✅ 呼出 |

> 💡 **Server Action** = 関数の先頭に `'use server'` を付けるとCC側からも呼び出せる仕組み。フォーム送信やボタン操作からDB更新を行う用途で使う（例：`<form action={createPost}>`）。後半で詳しく扱う
...
**登場する3ファイル：** `page.tsx`(SC親) / `ClientBoundary.tsx`(CC・トグル) / `StaticServerDataFetch.tsx`(SC・fetch)

```text
[① サーバー側（リクエスト時 or ビルド時）]
   page.tsx 実行 → <StaticServerDataFetch /> 完了 → 静的HTML
   → <ClientBoundary>(完成HTML)</ClientBoundary> として送信

[② ブラウザ：初回表示] 全体が即描画（ボタン・todo HTML揃った状態。まだ押せない）

[③ ハイドレーション] ClientBoundary の useState 起動 → ボタン操作可能に

[④ ボタンクリック] isVisible=true → 受け取り済みtodoが表示
   ※ 新しい fetch は走らない（サーバーで取得済みのHTMLを表示するだけ）
```
...
> 静的と動的の自動切り替えと、高速な描画を実現する裏側の仕組み
...
- **RSC Payload** の中身（SC/CC で何が違うか）と、ハイドレーション・画面遷移での役割を理解する
- `<Link>` による画面遷移がなぜ高速なのかを説明できる
- （オプション）`.next/` ディレクトリのビルド成果物を読み解ける

> 💡 用語の前提：**Payload** = 通信で送られるデータ本体／ **ISR** = Static のまま定期再生成する仕組み（5章で詳解）
> 🔗 3-15 で見た「SCの fetch はサーバーで1回だけ」という挙動が、本章の **Static Rendering** で完全に腑に落ちる
> 📚 章の流れ：**概念**（4-1〜4-8）→ **効用**（4-9 ハイドレーション・4-10 画面遷移）→ **実装詳細**（4-11〜4-13・任意）
...
## 4-4. Static / Dynamic の「自動判定」メカニズム
...
## 4-7. SC と CC で Payload に入るものが「正反対」
...
> 💡 **SCは「結果」、CCは「設計図 + 初期データ」**。どちらも同じPayloadに入るが、ブラウザでの扱いが正反対になる
...
> 🔑 これが3-3表の「**SCのJSはブラウザに送られず、CCのJSだけ送られる**」の正体。SC は Payload 内ですでにHTML化済みなので、関数本体を送る必要がない

---

## 4-9. RSC Payload の役割とハイドレーション

> Payload があるおかげで、**HTML即表示** と **ピンポイントなハイドレーション** が両立する
...
## 4-10. 画面遷移（ソフトナビゲーション）の仕組み

> `<Link>` による画面遷移時、Next.jsは**HTMLを再取得しない**。代わりに**RSC Payloadだけ**をリクエストする

| タイミング | 送受信するもの | 効果 |
|---|---|---|
| **初期アクセス** | 完全なHTML ＋ RSC Payload | 高速表示 ＆ SEOに強い |
...
## 4-13. 各ファイルの役割 — Static / Dynamic で何が変わる？
...
- **RSC Payload の二面性**: SC部分は**完成HTML**、CC部分は**JS参照+props**という正反対の構造を一つに格納（ハイドレーションの高効率化）
- **爆速の画面遷移**: `<Link>` 遷移時は HTML不要、Payloadだけ差分取得して更新
- **ビルド成果物は `.next/`**: Static は `page.html`+`page.rsc` を事前生成、Dynamic は `page.js` のみ（実装詳細）
...
> モダンなデータフェッチと、重い処理を待たせない滑らかなUI体験
...
> 🔗 4章で見た **Static / Dynamic レンダリング** の知識を前提に、本章では「実際にどうデータを取り、どう鮮度を保つか」を学ぶ
...
## 5-2. Server Component での極めてシンプルなデータ取得
...
> 🔑 **コード量が激減**。状態フック・ローディング判定が不要に。ブラウザにはデータ入りのHTMLがそのまま届くので**SEOにも強い**

---

## 5-2-2. 補足：型指定（`: User[]`）はなぜ書く？

> `res.json()` は何が返るか分からないので**デフォルトは `any` 型**（型チェックなし）。型を明示すると補完とタイポ検出が効く

```tsx
// ❌ 型を書かない場合：any 型なのでタイポしてもエラーにならない
const users = await res.json();   // users は any
users.map(u => u.nmae);           // 👈 "name" を "nmae" とタイポ → エラー出ず本番で発覚

// ✅ 型を明示する場合：データの「形」を定義して TypeScript に教える
type User = { id: number; name: string };  // 正しいプロパティ名は "name"
const users: User[] = await res.json();
users.map(u => u.nmae);           // 👈 "nmae" は User 型に存在しない → 即エラー検出
```

### 💡 メリット

- **エディタの自動補完** — `u.` と入力した瞬間に `id` / `name` の候補が表示される
- **タイポ検出** — `u.nmae` のような書き間違いを**保存時に即座に**指摘
- **データ構造の明文化** — `type User = {...}` を読めば「APIが返すデータの形」が分かる

> 🔑 **`type` 定義 + `: User[]` 明示**で、外部APIのデータも安全に扱える

---

## 5-2-3. TypeScript の型指定 — `type` 定義とジェネリクス

> useState や fetch 結果に型を指定すると、補完が効きタイポを即検出

```tsx
// ① 型エイリアス：データの「形」に名前をつける
type Todo = { id: number; todo: string };

// ② ジェネリクス：useState<T> で「中身の型」を教える
//    Todo | null = 「Todo か null」（ユニオン型 = | で OR）
const [todo, setTodo] = useState<Todo | null>(null);

// ▼ 使用例：型で守られる
setTodo({ id: 1, todo: "牛乳を買う" });  // ✅ 型と一致
setTodo({ id: 1, name: "牛乳" });        // ❌ "todo" が無い → 保存時にエラー

todo?.todo;   // ✅ 補完で .id / .todo が候補に出る
todo?.tdo;    // ❌ "tdo" は Todo に無い → タイポ検出
```

> 🔑 ジェネリクス `<T>` で型を教えると、`setter` も `getter` も型安全になる
...
> 💡 5-1で「SPAの辛さ」として登場した `useEffect + useState` だが、**CCで必要な場面に限定して使うのは現代でも正解**。次のスライドでまず実コードの全体像を見せ、その後でフックの基本を押さえる

---

## 5-3-2. CCのデータ取得 — 実コード解説 `ClientDataFetch.tsx`
...
## 5-5. フックのトリガー — いつ発火するか

> CCで状態管理とデータ取得を組み合わせる前提として、各フックの**発火タイミング**を理解する

### `useState` のトリガー — `setter` を呼んだ瞬間に再描画
...
setCount(1);             // ✅ 再描画トリガー → 画面が即更新
count = 1;               // ❌ 直接代入は React に検知されず、画面は変わらない
setCount(count + 1);     // ✅ 1 → 2 に更新して再描画
```
...
> 🔑 **useState は手動トリガー（`setter` 呼出）**、**useEffect は自動トリガー（依存変化）**。`fetch` は通常 `[]` で初回1回が鉄則
...
> 🔑 **`no-store` の正体は「Dynamic化スイッチ」**。データが新鮮になるのは "Dynamicに切り替わった結果" 毎リクエスト SC が動くから
...
> 💡 `<Suspense>` = React の機能。**非同期コンポーネントを待つ間、代替UI（fallback）を表示**してくれる仕組み
...
## 第5章まとめ — データ取得と Streaming SSR

**データを制する者は UX を制す**

- **SCがデフォルト**: `async`コンポーネント + `await fetch()` でデータ取得が直線的。状態フック不要・SEOに強い
...
- **Request Memoization** で同一リクエスト内の重複 `fetch` が**1回に集約**される仕組みと、その探索フロー（Memoization → Data Cache → 実通信）を理解する
...
> 💡 用語の前提：**メモ化（Memoization）** = 同じ計算結果を覚えておいて再利用する技法／ **revalidate** = キャッシュを再検証して更新すること
> 🔗 5-7 で見た `fetch` の `force-cache` / `no-store` / `revalidate` が、本章の **Data Cache 層**に対応している。本章はその全体像を体系化
...
| 4 | **Client Route Cache** | ブラウザメモリ | 同一ユーザーの遷移間 | `staleTimes` 設定値（30秒〜5分） |
...
## 6-2-2. `fetch` が呼ばれた時の探索フロー（Static / Dynamic 共通）

> Memoization → Data Cache → 実通信 の順で**毎回キャッシュを探す**
...
> 🔑 **Static は「ビルド時に fetch 完了 → デプロイに同梱」**、**Dynamic は「初回リクエストで fetch → 2回目以降キャッシュHIT」**。書き込み時刻が根本から違う
...
```
Dynamic：👤リクエスト → 🖥️SC実行 → 🗄️DB問合せ → HTML生成 → 返却（毎回調理）
Static ：🏗️ビルド時に1回だけ調理 → 💾保存 → 👤リクエストは保存品を即返却 ⚡
```
...
**🔄 作り置きを捨てるタイミング：** `revalidatePath('/about')`（手動）／`revalidate: 60` 期限切れ（自動）／次のデプロイ時

> 🔑 全員に同じものを返せるページは **SC実行ゼロ・DB通信ゼロ** で配信できる最大級の高速化
...
- **Request Memoization**: 1リクエスト内の同一URL `fetch` を自動で集約。`fetch` は **Memoization → Data Cache → 実通信** の順で探索される
...
- **結論**: 4層が連携することで **DB通信ゼロ・SC実行ゼロ・遷移待ちゼロ** の理想的なUXが成立する
...
> Next.js の自動化を支える「予約されたファイル名」を体系的に理解する
...
export default function RootLayout({
  ...
  return (
    <html lang="en"><body><Box>
      <span>Root Layout</span>
      {header}<NavigationHooks />{children}{modal}
    </Box></body></html>
  );
}
```
...
## 7-3-2. 補足：`loading.tsx` の正体 — 自動 `<Suspense>` ラップ

> SC内に `await` があると、その間 Next.js が `<Suspense fallback>` として loading.tsx を表示

**👨‍💻 あなたが書くコード（2ファイル置くだけ）：**

```tsx
// app/blog/loading.tsx
export default () => <p>📖 読み込み中…</p>;

// app/blog/page.tsx
export default async function BlogPage() {
  const posts = await fetch("/api/posts");  // ← await中は fallback 表示
  return <ul>{/* 投稿一覧 */}</ul>;
}
```

**⚙️ Next.js が裏で自動生成する構造：**
...
> 🔑 `loading.tsx` = `<Suspense fallback>` を**別ファイルに切り出した糖衣構文**。トリガーは「SC内の `await`」ただ1つ（同期SCやCCでは出ない）

---

## 7-4. `error.tsx` — エラー時に「ごめんなさい画面」を出す

> 💡 `<ErrorBoundary>` = React の機能。エラーで子が落ちても**画面全体ではなく一部だけ差し替える**仕組み。`error.tsx` がこれを自動配線
...
```
🛡️ error.tsx        → Header無傷で Pageだけ差替（普段はこっち）
🛡️ global-error.tsx → <html>ごと差替（Headerも消える・最終手段）
```

| ファイル | カバー範囲 | 発動する代表例 |
|---|---|---|
| `error.tsx` | layout の children 以下 | SC で `await fetch` が500・DB/Prisma失敗・throw |
| `global-error.tsx` | `<html>` から全部 | `layout.tsx` 内のSC失敗・Provider初期化失敗 |

```tsx
// src/app/global-error.tsx — Headerごと壊れた時の最終手段
'use client'
...
> ⚠️ `notFound()` 呼出は error.tsx ではなく **`not-found.tsx`** に飛ぶ（次スライド参照）
...
> 各スロットは **URL一致時 → `page.tsx`、それ以外 → `default.tsx`** が描画される

```tsx
// src/app/layout.tsx — 各スロットを引数で受け取り並べる
...
**📁 本プロジェクトの @スロット 配置：**
- `@header/`：`default.tsx`（"Default Header"）／`dynamic-rendering/page.tsx`（URL一致時）
- `@modal/`：`default.tsx`（`null`）／`(.)intercepting-route/page.tsx`（リンク経由）

| URL | header スロット | modal スロット |
|---|---|---|
| `/` | Default Header | （null） |
| `/dynamic-rendering` | DynamicRenderingHeader | （null） |
| `/intercepting-route`（リンク） | Default Header | ✅ モーダル |
| `/intercepting-route`（直） | Default Header | （null） |

> 🔑 **各スロットに `default.tsx` を必ず置く**。無いとリロード時に404を起こす（既定UIのフォールバックが無いため）
